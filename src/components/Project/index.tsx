  ...
  return (
    <div className="  bg-[url('/exp-gradient.svg')] bg-no-repeat bg-center">
      {props.projects.map((proj: Project, index: number) => {
        ...
        return (
          <div
            className={`flex flex-col mt-[50px] lg:mt-[100px] ${
              isTextLeft ? 'lg:flex-row' : 'lg:flex-row-reverse'
            } relative w-fit ${poppin.className}`}
            key={'project' + index}
          >
            <div
              className={`gap-[30px] flex flex-col lg:w-[50%] z-20 ${
                isTextLeft ? 'items-start' : 'items-end'
              }`}
            >
              <div>
                <p
                  className={`text-[#9857D3] text-[16px] font-semibold ${
          ...
          <img src='/click.svg' alt='click' />
            ...
            <div
              className={`bg-[#2B0B3A] ${
                isTextLeft ? ' lg:pl-[30px] lg:ml-[-5%]' : 'lg:pr-[30px] lg:mr-[-5%]'
